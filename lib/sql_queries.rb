...
def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
  # "SELECT projects.title, SUM(pledges.amount - projects.funding_goal) FROM projects INNER JOIN pledges ON projects.id = pledges.project_id GROUP_BY SUM(pledges.amount);"
  "SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS leftover FROM projects INNER JOIN pledges ON projects.id = pledges.project_id GROUP BY projects.title HAVING leftover >= 0;"
end
